...
namespace {
    // value and error testing types

    struct ipair {
        int a, b;
        ipair(): a(0), b(0) {}
        ipair(int a, int b): a(a), b(b) {}
    };
    ...
    struct foo {
        ...
        foo(foo&& f): v(f.v), was_move_constructed(true) {}

        foo(const ipair& p): foo(p.a, p.b) {}
        foo(ipair&& p): v(p.a+p.b), was_move_constructed(true) {}
    };
}
...
TEST(expected, ctors) {
    ...
    expected<foo, foo> e1;
    ASSERT_TRUE(e1);
    EXPECT_EQ(-1, e1.value().v);

    // from value type
    foo f2(3, 5);
    expected<foo, foo> e2(f2);
    ASSERT_TRUE(e2);
    EXPECT_EQ(8, e2.value().v);
    EXPECT_FALSE(e2.value().was_move_constructed);

    expected<foo, foo> e3(std::move(f2));
    ASSERT_TRUE(e3);
    EXPECT_EQ(8, e3.value().v);
    EXPECT_TRUE(e3.value().was_move_constructed);

    // from a compatible value type
    ipair p4(7, 8);
    expected<foo, foo> e4(p4);
    ASSERT_TRUE(e4);
    EXPECT_EQ(15, e4.value().v);
    EXPECT_FALSE(e4.value().was_move_constructed);

    expected<foo, foo> e5(ipair(2, 3));
    ASSERT_TRUE(e5);
    EXPECT_EQ(5, e5.value().v);
    EXPECT_TRUE(e5.value().was_move_constructed);
}

#if 0
// Tests taken from arbor expected tests: rewrite.
